...
public class WebServerSSE
{
    ...
    private void connect(WebServerContext context)
    {
        ...
        dispatchEvent(WebServerSSEEventType.CONNECT,
                new WebServerSSEEvent(WebServerSSEEventType.CONNECT, clientId, null));
    }
    ...
    private void subscribe(WebServerContext context)
    {
        ...
        dispatchEvent(WebServerSSEEventType.SUBSCRIBE,
                new WebServerSSEEvent(WebServerSSEEventType.SUBSCRIBE, clientId, channelName));
    }
    ...
    private void unsubscribe(WebServerContext context)
    {
        ...
        dispatchEvent(WebServerSSEEventType.UNSUBSCRIBE,
                new WebServerSSEEvent(WebServerSSEEventType.UNSUBSCRIBE, clientId, channelName));
    }
    ...
    private void emit(OutputStream stream, String channel, String message)
    {
        ...
        try
        {
            stream.write(String.format("event: %s\n", channel).getBytes("UTF-8"));
            stream.write(String.format("data: %s", message).getBytes("UTF-8"));
            stream.write("\n\n".getBytes());
            stream.flush();
        }
        catch (Exception e)
